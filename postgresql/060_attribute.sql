...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION qgis_pkg.collect_nested_attribute(varchar, varchar, integer, text, varchar) IS 'Collect the "inline" attribute value in specific cdb_schema';
REVOKE EXECUTE ON FUNCTION qgis_pkg.collect_nested_attribute(varchar, varchar, integer, text, varchar) FROM public;
-- Example
-- SELECT * FROM qgis_pkg.collect_nested_attribute('qgis_bstsai', 'citydb', 901, 'height'); -- single height entry
-- SELECT * FROM qgis_pkg.collect_nested_attribute('qgis_bstsai', 'citydb', 15, 'relatedTo'); -- single height entry
...
DROP FUNCTION IF EXISTS qgis_pkg.get_attribute_key_id(varchar, integer, varchar, boolean);
...
IF NOT is_nested THEN
	...
	IF attri_id IS NULL THEN
		RAISE NOTICE 'The attribute % of objectclass_id % can not be found! Please check if the existence of the attribute in schema %', qi_attri_name, oc_id, ql_cdb_schema;
	ELSE
		RETURN attri_id;
	END IF;
ELSE
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION qgis_pkg.get_attribute_key_id(varchar, integer, varchar, boolean) IS 'Lookup the primary key id of the given attribute. If the attribute is nested, it will return the key id of the first id of its child attribute';
REVOKE EXECUTE ON FUNCTION qgis_pkg.get_attribute_key_id(varchar, integer, varchar, boolean) FROM public;
-- Example
-- SELECT * FROM qgis_pkg.get_attribute_key_id('citydb', 901, 'name');
-- SELECT * FROM qgis_pkg.get_attribute_key_id('citydb', 901, 'height', TRUE);
-- SELECT * FROM qgis_pkg.get_attribute_key_id('citydb', 901, '土砂災害警戒区域', TRUE);
...
DECLARE
    ...
    av_name CONSTANT varchar := trim(both '"' from qi_av_name);
	attri_id bigint := (SELECT qgis_pkg.get_attribute_key_id(cdb_schema, objectclass_id, attribute_name, is_nested));
    idx_name varchar;
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION qgis_pkg.generate_sql_attribute_matview_footer(varchar,varchar,varchar,varchar,integer,varchar,boolean) IS 'Generate the footer for creating indices on every attribute materialized view value columns and refresh it';
REVOKE EXECUTE ON FUNCTION qgis_pkg.generate_sql_attribute_matview_footer(varchar,varchar,varchar,varchar,integer,varchar,boolean) FROM public;
--Example
-- SELECT * FROM qgis_pkg.generate_sql_attribute_matview_footer('bstsai', 'qgis_bstsai', '"citydb_attri_v_nested_height"', 'citydb', 901, 'height',TRUE);
...
IF sql_attri IS NOT NULL THEN
	...
	END IF;
ELSE
	RAISE NOTICE 'The sql_attri is null. Please check the existence of attribute values in schema %', cdb_schema;
END IF;
