...
(
	SELECT 
		...
		CASE o.classname
			...
			WHEN 'ClosureSurface' 				THEN 'closuresurf'
			WHEN 'CityModel'					THEN 'citymodel'
			WHEN 'ImplicitGeometry'				THEN 'implicit_geom'
			WHEN 'GenericLogicalSpace' 			THEN 'gen_logical_space'
			WHEN 'GenericOccupiedSpace' 		THEN 'gen_occupied_space'
			WHEN 'GenericUnoccupiedSpace' 		THEN 'gen_unoccupied_space'
			WHEN 'GenericThematicSurface' 		THEN 'gen_thematic_surf'
			...
			WHEN 'Intersection' 				THEN 'trn_intersection'
			WHEN 'Square' 						THEN 'trn_sqare'
			WHEN 'Track' 						THEN 'trn_track'
			WHEN 'Road' 						THEN 'trn_road'
			WHEN 'AuxiliaryTrafficSpace' 		THEN 'trn_auxilary_traffic_space'
			WHEN 'ClearanceSpace' 				THEN 'trn_clearance_space'
			WHEN 'TrafficSpace' 				THEN 'trn_traffic_space'
			WHEN 'Hole' 						THEN 'trn_hole'
			WHEN 'AuxiliaryTrafficArea' 		THEN 'trn_auxilary_traffic_area'
			WHEN 'TrafficArea' 					THEN 'trn_traffic_area'
			...
			WHEN 'BridgeFurniture' 				THEN 'bri_frn'
 			WHEN 'CityObjectGroup' 				THEN 'cityobj_grp' -- no need to instantiate
		...
		o.ade_id,
		o.namespace_id
	FROM citydb.objectclass AS o
		INNER JOIN citydb.namespace AS n ON o.namespace_id = n.id
	WHERE is_abstract = 0
...
CREATE TABLE qgis_pkg.attribute_datatype_lookup AS(
	SELECT 
		...
		END AS val_col
	FROM citydb.datatype AS d
		INNER JOIN citydb.namespace AS n ON d.namespace_id = n.id
	ORDER BY d.id
);
