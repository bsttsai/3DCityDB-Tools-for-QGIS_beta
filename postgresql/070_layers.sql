...
ELSE
	...
	LOOP
		attri_id  := (SELECT * FROM qgis_pkg.get_attribute_key_id(r.cdb_schema, r.objectclass_id , r.parent_attribute_name, qi_is_nested));
...
IF attri_view_name IS NOT NULL THEN
	...
	IF ARRAY_LENGTH(view_col_names,1) > 0 THEN
		...
		LOOP
			val_col_type := qgis_pkg.get_view_column_type(qi_usr_schema, attri_view_name, view_col_name);
			IF val_col_type <> 'json' THEN
				attri_idx_name := concat('"', qi_lv_name_idx, '_a_col_', attri_count, '"');
sql_statement := concat(sql_statement,'
CREATE INDEX ',attri_idx_name,' ON ',qi_usr_schema,'."',qi_lv_name,'" ("',view_col_name,'");');
			END IF;
			attri_count := attri_count + 1;
...
inline_attri_ids := qgis_pkg.get_all_attribute_id_in_schema(qi_cdb_schema, oc_id);
nested_attri_ids := qgis_pkg.get_all_attribute_id_in_schema(qi_cdb_schema, oc_id, TRUE);
...
-- SELECT * FROM qgis_pkg.get_attribute_columns((SELECT qgis_pkg.collect_inline_attribute('qgis_bstsai', 'citydb', 901, 'name')))
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION qgis_pkg.check_attribute_column_is_json(text) IS 'Check whether the value columns from a collect attribute query is json type';
...
----------------------------------------------------------------
-- The function create the view or materialized view of selected feature attributes
...
sql_atv_select_f_id := concat(sql_atv_select_f_id, 'a',i,'.',attri_val_cols[j],',');
				ELSE
sql_atv_select_val := concat(sql_atv_select_val,'
	a',i,'.',attri_val_cols[j],',');
...
CREATE INDEX "', qi_cdb_schema,'_', objectclass_id, '_g_', geometry_id, '_attri_', (trim (both '"' FROM attri_val_cols[j])),'_idx" ON ', qi_usr_schema, '.', view_name, ' (',attri_val_cols[j],');');
			...
			IF i - 1 > 1 THEN
sql_atv_full_join_key := concat(sql_atv_full_join_key, ' a', i-1, '.f_id,');
			END IF;

sql_atv_from := concat(sql_atv_from,'
FULL JOIN (', sql_attri, ') AS a',i, ' 
	ON ', LEFT(sql_atv_full_join_key, LENGTH(sql_atv_full_join_key)-1),') = a', i, '.f_id');
		END LOOP;
	END IF;
...
--Example
-- SELECT * FROM qgis_pkg.create_attris_table_view('qgis_bstsai', 'citydb', 901, ARRAY[2]); -- name
...
-- SELECT * FROM qgis_pkg.create_attris_table_view('qgis_bstsai', 'rh_v5', 901, NULL, TRUE, TRUE); -- all automatic
...
-- Example
-- SELECT * FROM qgis_pkg.create_layer_attri_table('qgis_bstsai', 'citydb', 30, NULL, FALSE, TRUE); -- view: citydb all bdg lod1solid with all attris
...
-- SELECT * FROM qgis_pkg.create_layer_attri_table('qgis_bstsai', 'citydb', 30, ARRAY[108,114,110], TRUE); -- citydb all bdg lod1solid with 'description','name', 'function'
...
----------------------------------------------------------------
DROP FUNCTION IF EXISTS    qgis_pkg.create_layer(varchar, varchar, integer, integer, text, text[], boolean, boolean, boolean) CASCADE;
...
DECLARE
	...
	qi_usr_schema			varchar 	:= quote_ident(usr_schema);
	qi_cdb_schema 			varchar		:= quote_ident(cdb_schema);
	geom_id  				integer 	:= qgis_pkg.get_geometry_key_id(qi_cdb_schema, p_oc_id, oc_id, geom);
	attri text;
	attri_id integer;
	attri_ids integer[];
	sql_layer text;
...
IF attris IS NOT NULL AND NOT is_all_attri THEN
	...
	LOOP
		attri_id := qgis_pkg.get_attribute_key_id(qi_cdb_schema, oc_id, attri);
		attri_ids := ARRAY_APPEND(attri_ids, attri_id);
	END LOOP;
END IF;
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION qgis_pkg.create_layer(varchar, varchar, integer, integer, text, text[], boolean, boolean, boolean) IS 'Create feature class layer by joining geometry matview with either multiple join or table of attribute views';
REVOKE EXECUTE ON FUNCTION qgis_pkg.create_layer(varchar, varchar, integer, integer, text, text[], boolean, boolean, boolean) FROM public;
-- Example
--(usr_schema, cdb_schema, p_oc_id, oc_id, geom_name, attribute_name[], is_matview, is_all_attri, is_joins)
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid'); -- VIEW geom only 
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', NULL, TRUE); -- MATVIEW geom only
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', ARRAY['name','description'], FALSE, FALSE); -- VIEW g+a (TABLE) 
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', ARRAY['name','description'], TRUE, FALSE); -- MATVIEW g+a (TABLE)
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', ARRAY['name','description'], FALSE, FALSE, TRUE); -- VIEW g+a (JOIN)
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', ARRAY['name','description'], FALSE, TRUE, TRUE); -- MATVIEW g+a (JOIN)
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', NULL, FALSE, TRUE); -- VIEW g+ ALL a (TABLE)
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', NULL, TRUE, TRUE); -- MATVIEW g+ ALL a (TABLE)
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', NULL, FALSE, TRUE, TRUE); -- VIEW g+ ALL a (JOIN)
-- SELECT * FROM qgis_pkg.create_layer('qgis_bstsai', 'citydb', 0, 901, 'lod1Solid', NULL, TRUE, TRUE, TRUE); -- MATVIEW g+ ALL a (JOIN)